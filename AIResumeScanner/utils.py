try:
    ...
    class _MissingStreamlit:
        def __getattr__(self, name):
            raise ImportError("streamlit is required for UI rendering")
...
def ai_chatbot_response(prompt: str) -> str:
    ...
    if not prompt:
        return "Ask me about resumes, skills, or job requirements, and I’ll help."
...
def display_resume_details(resume_data):
    ...
    all_skills = resume_data.get('skills', []) or []
    matching_skills = resume_data.get('matching_skills', []) or []
    ...
    with col2:
        st.write("**Other**")
        matching_lookup = {str(skill).lower() for skill in matching_skills}
        other_skills = [skill for skill in all_skills if str(skill).lower() not in matching_lookup]
        if other_skills:
            for skill in other_skills[:10]:  # Show only top 10 other skills
                st.write(f"- {skill}")
            if len(other_skills) > 10:
                st.write(f"- ... +{len(other_skills) - 10}")
        else:
            st.write("None")

    # Display resume preview without nested expander
    resume_text = resume_data.get('text', '') or ""
    preview_text = resume_text[:1000] + ("..." if len(resume_text) > 1000 else "")
    st.text_area("Preview", preview_text, height=180, label_visibility="collapsed")
...
def get_top_keywords(resumes_data, job_skills, top_n=10):
    ...
    """
    if pd is None:
        raise ImportError("pandas is required to build the keywords table")

    # Count skill frequencies across all resumes
    skill_counter = Counter()
    job_skill_set = {str(skill).lower() for skill in (job_skills or [])}

    for resume in resumes_data:
        resume_skills = [str(skill).lower() for skill in (resume.get('skills', []) or [])]
...
def generate_chatbot_response(query, ranked_resumes, job_skills):
    ...
    response = "Try: details for candidate 1, compare 1 and 2, or who has Python." # Default response
    ...
    def _safe_resume_value(resume, key, default=""):
    ...
    if detail_match:
        ...
        if candidate_index_str:
            try:
                ...
                if 0 <= idx < len(ranked_resumes):
                    ...
                    filename = _safe_resume_value(resume, 'filename', 'Unknown')
                    response = f"**Candidate {idx + 1}: {filename}**\\n"
                    response += f"- **Match:** {resume.get('match_percentage', 0)}%\\n"
                    matching_skills = resume.get('matching_skills', []) or []
                    matching_lookup = {str(skill).lower() for skill in matching_skills}
                    matching_skills_str = ', '.join(matching_skills) if matching_skills else 'None'
                    response += f"- **Fit:** {matching_skills_str}\\n"
                    other_skills = [skill for skill in (resume.get('skills', []) or []) if str(skill).lower() not in matching_lookup]
                    other_skills_str = ', '.join(other_skills[:5]) + ('...' if len(other_skills) > 5 else '') if other_skills else 'None'
                    response += f"- **Other:** {other_skills_str}"
                    found = True
            except ValueError:
             ...
             for i, resume in enumerate(ranked_resumes):
                 resume_filename = str(resume.get('filename', '')).lower()
                 if filename_lower == resume_filename: # Exact match preferred
                    response = f"**Candidate {i + 1}: {resume.get('filename', 'Unknown')}**\\n"
                    response += f"- **Match:** {resume.get('match_percentage', 0)}%\\n"
                    matching_skills = resume.get('matching_skills', []) or []
                    matching_lookup = {str(skill).lower() for skill in matching_skills}
                    matching_skills_str = ', '.join(matching_skills) if matching_skills else 'None'
             ...
             if not found:
                 for i, resume in enumerate(ranked_resumes):
                     resume_filename = str(resume.get('filename', '')).lower()
                     if filename_lower in resume_filename:
                        response = f"Found partial match: **Candidate {i + 1}: {resume.get('filename', 'Unknown')}**\\n"
                        response += f"- **Match:** {resume.get('match_percentage', 0)}%\\n"
                        matching_skills = resume.get('matching_skills', []) or []
                        matching_lookup = {str(skill).lower() for skill in matching_skills}
                        matching_skills_str = ', '.join(matching_skills) if matching_skills else 'None'
        ...
        if not found:
             response = f"Could not find '{target_query}'. Try a candidate number or exact filename."
        return response
    ...
    if compare_match:
        try:
            ...
            if 0 <= idx1 < len(ranked_resumes) and 0 <= idx2 < len(ranked_resumes):
                ...
                res1_name = res1.get('filename', 'Unknown')
                res2_name = res2.get('filename', 'Unknown')
                res1_matching = res1.get('matching_skills', []) or []
                res2_matching = res2.get('matching_skills', []) or []
                response = f"**Comparison: {res1_name} (Candidate {idx1+1}) vs {res2_name} (Candidate {idx2+1})**\\n"
                response += f"- **Match %:** {res1.get('match_percentage', 0)}% vs {res2.get('match_percentage', 0)}%\\n"
                response += f"- **Fit:** {len(res1_matching)} vs {len(res2_matching)} skills\\n"
                common_skills = set(res1_matching).intersection(set(res2_matching))
                response += f"- **Common:** {', '.join(common_skills) if common_skills else 'None'}\\n"
                res1_unique = set(res1_matching) - common_skills
                res2_unique = set(res2_matching) - common_skills
                response += f"- **{res1_name}:** {', '.join(res1_unique) if res1_unique else 'None'}\\n"
                response += f"- **{res2_name}:** {', '.join(res2_unique) if res2_unique else 'None'}"
            else:
    ...
    if skill_match:
        ...
        for i, resume in enumerate(ranked_resumes):
            ...
            resume_skills = [str(s).lower() for s in (resume.get('skills', []) or [])]
            if skill_query in resume_skills:
                 # Check if it's a matching skill for extra info
                 matching_skills = [str(s).lower() for s in (resume.get('matching_skills', []) or [])]
                 is_matching = skill_query in matching_skills
                 match_info = "(Matching Skill)" if is_matching else "(Other Skill)"
                 candidates_with_skill.append(f"Candidate {i+1}: {resume.get('filename', 'Unknown')} (Match: {resume.get('match_percentage', 0)}%) {match_info}")

        if candidates_with_skill:
            response = f"With **{skill_query}**:\\n- " + "\\n- ".join(candidates_with_skill)
        else:
            response = f"No candidates have **{skill_query}**."
        return response
        ...
        try:
            ...
            if n > 0:
                ...
                for i, resume in enumerate(ranked_resumes[:n]):
                     response += f"{i+1}. {resume.get('filename', 'Unknown')} (Match: {resume.get('match_percentage', 0)}%)\\n"
    ...
    if query_lower in ["hi", "hello", "help", "what can you do?"]:
        response = "Try: top 3, details for candidate 1, compare 1 and 2, or who has Python."
        return response
